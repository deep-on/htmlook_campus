"""
Fourier Transform · extract the spectrum from a time-domain signal via winding.
...
class FourierIntuition(Scene):
    def construct(self):
        title = Text("Fourier Transform · Intuition", font_size=36)
        sub = Text("time ↔ frequency", font_size=22, color=GRAY).next_to(title, DOWN)
        ...
        narration = Text(
            "Recovers the exact strength of the two frequencies (3, 7)",
            font_size=24,
            color=WHITE,
