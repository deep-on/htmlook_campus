...
`create_file` 은 `apply_edit` 와 달리 워크스페이스 루트 안인지 검사하지 않음 — 절대 또는 워크스페이스-상대 경로 전달. 워크스페이스 안 유지는 caller 책임.
...
HTMLook 이 `apply_edit` (또는 어떤 write tool 이든) 실행을 허락하기 전에 사용자는 허가 모달을 봅니다:
...
| **거부** | 이번 호출만 거부 |
| **한 번만 허용** | 이번 호출만 허용 |
| **항상 (워크스페이스)** | 이 워크스페이스에서 이 tool 의 미래 호출 허용 |
| **항상 (전역)** | 모든 워크스페이스에서 이 tool 의 미래 호출 허용 |

### Tool 이 카테고리로 매핑되는 방식
